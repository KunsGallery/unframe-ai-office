    ...
    <section className={`office-map-shell theme-${room.theme}`}>
      <div className="pixel-office-header">
        ...
        <div className="pixel-office-guide">
          <span className="guide-desktop">방향키·WASD·ㅈㅁㄴㅇ 이동 · 자리나 AI 앞에서 Enter</span>
          <span className="guide-mobile">AI 직원을 터치해 대화할 수 있습니다.</span>
        </div>
      </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      left: `${layout.deskX}%`,
                      top: `${layout.deskY}%`,
                      "--desk-accent": agent.character?.accentColor || agent.color,
                    }}
                  >
              ...
              {agents.map((agent) => {
                ...
                return (
                  <OfficeAgent
                    ...
                    status={
                      visualState?.status ||
                      getAgentVisualStatus(agent.id, activeAgentId, nearestCharacterId)
                    }
      ...
      <div className="office-map-help">
        <span>AI 현재 위치와 데스크 위치를 함께 감지해 Enter 상호작용을 처리합니다.</span>
        <span>자리 비움 상태의 AI는 자기 데스크로 돌아온 뒤 대화가 시작됩니다.</span>
      </div>
    </section>
  );
}
