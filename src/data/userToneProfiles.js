  ...
  if (!email || isOwnerEmail(email)) {
    return {
      ...
      toneInstruction: [
        ...
        "기본 답변은 짧고 압축적으로 정리하고, 사용자가 길게 요청할 때만 자세히 확장한다.",
      ].join(" "),
    };
  }
  ...
  if (isSoyeonEmail(email) || email) {
    return {
      ...
      toneInstruction: [
        ...
        "답변은 부드럽더라도 길게 늘이지 말고 핵심만 짧게 전달한다.",
      ].join(" "),
    };
  }
...
export function buildToneInstruction(userOrEmail) {
  ...
  if (profile.key === "soyeon") {
    return [
      ...
      "- 장황한 설명 대신 핵심만 짧게 전달하고, 길게 요청받을 때만 확장한다.",
  ...
  return [
    "호칭/말투 규칙:",
    `- ${profile.toneInstruction}`,
    "- 기본 응답은 간결하게 하고, 길게 요청받을 때만 자세히 확장한다.",
    "- 모든 응답은 담당 agent 역할에 맞는 전문성을 유지한다.",
  ].join("\n");
}
